...
namespace s21 {
    ...
    std::list<EquationMember *>  StringTransformer::StringProcessing(std::string& prepared_string) {
    ...
    void StringTransformer::CheckX(std::list<EquationMember *>& list, const char *str) {
    ...
    Utility::OperationType StringTransformer::CheckUnary(const std::string& str2, Utility::OperationType type) noexcept {
        Utility::OperationType new_type = type;
        if (str2.empty() || !(s21::Utility::IsNumber(str2) || s21::Utility::IsRightBracket(str2))) {
            switch (type) {
                case Utility::kPlus:
    ...
    void StringTransformer::CheckBracket(std::list<EquationMember *>& list, const char *str) {
    ...
    void StringTransformer::CheckFunction(std::list<EquationMember *>& list, const std::string& str) {
    ...
    void StringTransformer::CheckSign(std::list<EquationMember *>& list, const char *str, const std::string& str2) {
    ...
    void StringTransformer::SortingStation(EquationMember *member) {
        if (member->GetValueType() == Utility::kNumber || member->GetValueType() == Utility::kX) {
        ...
        if (!stack_operation_.empty()) {
            ...
            if (new_member->GetValueType() == Utility::kLeftBracket) {
                ...
                while (!stack_operation_.empty()) {
                    if (stack_operation_.back()->GetPriority() >= new_member->GetPriority()) {
