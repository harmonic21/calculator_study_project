...
class CalculationController {
 public:
  explicit CalculationController(CalculateModel* model)
      : calculateModel_(model) {}

  CalculationController(const CalculationController& other) {
    calculateModel_ = other.calculateModel_;
  }

  CalculationController(CalculationController&& other) noexcept
      : calculateModel_(nullptr) {
    *this = std::move(other);
  }
